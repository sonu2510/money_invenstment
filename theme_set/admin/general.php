...
class general extends dbclass{
	...
	public function getTotalMenuCout(){
		...
		//print_r('fWFGWE');//die;
		$sql = "SELECT COUNT(*) as total FROM  admin_menu` WHERE parent_id = '0' AND status = '1' ";
//print_r($sql);die;
			...
			for($i=0;$i<$sub_menu_data->num_rows;$i++) {
				...
				if($this->checkUser()){
					...
					if ($this->hasPermission('view', $sub_menu[$i]['admin_menu_id'])) {
						...
						if($sub_menu[$i]['admin_menu_id']!=ORDER_ACCEPT_ID && $sub_menu[$i]['admin_menu_id']!=ORDER_PRICEEDIT_ID && $sub_menu[$i]['admin_menu_id']!=DISCOUNT_RATE_PERMISSION && $sub_menu[$i]['admin_menu_id']!=GRESS_PRICE_PERMISSION && $sub_menu[$i]['admin_menu_id']!=LEAD_LIST_PERMISSION && $sub_menu[$i]['admin_menu_id']!=SHOW_CHART_PERMISSION && $sub_menu[$i]['admin_menu_id']!=SHOW_THICKNESS_PERMISSION  && $sub_menu[$i]['admin_menu_id']!=PHYSICAL_STOCK_PERMISSION)

						{

    						$return .= '<li ><a  class="" href="' .$set_rout. '">'.$sub_menu[$i]['name'].'</a>';
