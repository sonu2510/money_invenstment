            ...
            <div class="bootstrap-data-table-panel">
              ...
              <thead>
                <tr>
                  ...
                  <th>Date</th>               
                  <th>Amount</th>               
                  <th>View Amount Details</th>
                </tr>
              </thead>
              <tbody>
              <?php
           
				  $added_data = $obj_money->getMembers();
				  if($added_data) {
          				  foreach($added_data as $data){           					
          					?>
                              <tr>
                                <td><input type="checkbox" name="post[]" value="<?php echo $data['member_id'];?>"></td>
                                <td><?php echo  $data['member_name'];?></td>
                                  <td><?php echo dateFormat(4,$data['date']);?></td>
                                  <td><?php echo  $data['total_amount'];?></td>  
                                  <td>
                                	<a href="<?php echo $obj_general->link($rout, 'mod=view&member_id='.encode($data['member_id']).'&filter_edit='.$filter_edit, '',1); ;?>"  name="btn_edit" class="btn btn-info btn-rounded m-b-10 m-l-5">View</a>
