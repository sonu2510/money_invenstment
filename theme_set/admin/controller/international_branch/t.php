          ...
          <header class="panel-heading"> 
      		  ...
      		  <span class="text-muted m-l-small pull-right">
                    ...
                    <?php if($obj_general->hasPermission('add',$menuId)){ ?>
                    		<a class="label bg-primary" href="<?php echo $obj_general->link($rout, 'mod=add', '',1);?>"><i class="fa fa-plus"></i> New International </a>
          ...
          <form name="form_list" id="form_list" method="post">
           <input type="hidden" id="action" name="action" value="" /> 
            ...
            <table class="table table-striped b-t text-small">
              ...
              <tbody>
              <?php
              $total_branch = $obj_branch->getTotalBranch($filter_data);
			  $pagination_data = '';
			  if($total_branch){
				   	if (isset($_GET['page'])) {
						$page = $_GET['page'];
				  ...
				  $branchs = $obj_branch->getBranchs($option,$filter_data);
				  if($branchs) {
				  foreach($branchs as $branch){ 
                    ...
                    <tr>
                      ...
                      <td>
                      		<a href="<?php echo $obj_general->link($rout, 'mod=add&branch_id='.encode($branch['international_branch_id']).'&filter_edit='.$filter_edit, '',1); ;?>"  name="btn_edit" class="btn btn-info btn-xs">Edit</a>
                            <a href="<?php echo $obj_general->link($rout, 'mod=permission&branch_id='.encode($branch['international_branch_id']), '',1);?>"  name="btn_permission" class="btn btn-warning btn-xs">Permission</a>
