...
class money_investment extends dbclass{
	...
	public function updateMoney($added_money_id,$data){	
		...
		$sql = "UPDATE `" . DB_PREFIX . "add_money SET  member_name = '" . $data['member_name'] . "', member_id = '" . $member_id . "',date='".$data['date']."',amount='".$data['amount']."',remarks='".$data['remarks']."',date_added = '".date('Y-m-d H:i:s')."' ,date_modify ='".date('Y-m-d H:i:s')."' WHERE added_money_id = '".(int)$added_money_id."'";
		//echo $sql;die;
		$this->query($sql);				
