...
//}
	...
	//print_r($data);die;
		...
		if(!isset($data['igst_status']))
			$data['igst_status']='0';
    	if(!isset($data['show_cylinder_air']))
			$data['show_cylinder_air']='0';
		if(!isset($data['show_air_f']))
			$data[show_air_f]='0';
		if(!isset($data['FOB_value']))
			$data['FOB_value']='0';	
		if(!isset($data['license_text']))
			$data['license_text']='';
...
//change by sonu 06-03-2018
	...
	{
		$sql="SELECT * FROM invoice_product_test WHERE invoice_id='".$invoice_no."' ORDER BY product_id ASC";
		if($n==1)
		    $sql="SELECT * FROM invoice_product_test WHERE invoice_id='".$invoice_no."' AND product_id NOT IN (10,23,11,18,6,34,47,48,63,72,70) ORDER BY product_id ASC";
	...
	public function gettotalProductCategory($invoice_id)
	{
	    $data=$this->query("SELECT product_id FROM invoice_product_test WHERE invoice_id='".$invoice_id."' AND product_id IN (10,23,11,18,6,34,47,48,63,72,37,38,70) GROUP BY product_id  ORDER BY product_id ASC");
	    if($data->num_rows)
			return $data->rows;
		else
			return '1';
	...
	//	printr(decode($invoice['transportation']));
	  ...
	  //  printr($pallet_details);
		...
		$scoop_no = $roll_no = $mailer_no = $sealer_no = $storezo_no = $paper_no = $con_no =$gls_no = $val_no =$chair_no=$silica_gel_no =$oxygen_absorbers_no =$straw_no = 0;
		
		//sonu add 15-5-2017
		$scoop_box_no = $roll_box_no = $mailer_box_no = $sealer_box_no = $storezo_box_no = $paper_box_no= $pouch_box_no = $con_box_no = $gls_box_no = $val_box_no =$chair_box_no=$silica_gel_box_no=$oxygen_absorbers_box_no =$straw_box_no =0;
		$scoop_name = $roll_name = $mailer_name = $sealer_name = $storezo_name = $paper_box_name=$pouch_name = $con_box_name=$gls_box_name=$val_box_name=$chair_box_name=$silica_gel_box_name=$oxygen_absorbers_box_name=$straw_box_name='';
		//end
		$scoop_series = $roll_series = $mailer_series = $sealer_series = $storezo_series = $paper_series = $con_series = $gls_series = $val_series =$chair_series =$silica_gel_series=$oxygen_absorbers_series =$straw_series ='';		
		$total_amt_scoop = $total_amt_roll = $total_amt_mailer = $total_amt_sealer = $total_amt_storezo = $total_amt_paper = $total_amt_con =$total_amt_gls =$total_amt_val =$total_amt_valve=$total_amt_chair=$total_amt_silica_gel=$total_amt_oxygen_absorbers =$total_amt_straw = 0;
		//sonu add 15-5-2017
		$total_net_w_scoop =$total_gross_w_scoop = $total_net_w_roll=$total_gross_w_roll= $total_net_w_p =$total_gross_w_p  = $total_net_w_m =$total_gross_w_m= $total_net_w_s =$total_gross_w_s =$total_net_w_str =$total_gross_w_str =$total_net_w_pouch=$total_gross_w_con =$total_net_w_con=$total_gross_w_gls =$total_net_w_gls=$total_gross_w_val =$total_net_w_val=$total_net_w_chair=$total_net_w_silica_gel=$total_net_w_oxygen_absorbers=$total_net_w_straw=0;
		//end
    	...
    	if($details['product_id']!='11' && $details['product_id']!='6' && $details['product_id']!='10' && $details['product_id']!='23' && $details['product_id']!='18' && $details['product_id']!='34' && $details['product_id']!='47' && $details['product_id']!='48' && $details['product_id']!='63' && $details['product_id']!='72' && $details['product_id']!='38' && $details['product_id']!='37' && $details['product_id']!='70')
		{
    	  $abcd = 'B'; 
		}else{
		  $abcd = 'A';
		}
	    $total_items = $this->gettotalProductCategory($invoice_no);
		//printr($total_items);
		foreach($total_items as $item)
		{
		    if($item['product_id']==11)
		       ...
		       $mailer_series = $abcd;
		    else if($item['product_id']=='23')  
		       $sealer_series = $abcd;
		    else if($item['product_id']=='18')  
		       $storezo_series = $abcd;
		    else if($item['product_id']=='34')  
		       $paper_series = $abcd;
		    else if($item['product_id']=='47')  
		       $con_series = $abcd;
		    else if($item['product_id']=='48')  
		       $gls_series = $abcd;
		    else if($item['product_id']=='72')  
		       $chair_series = $abcd;
		    else if($item['product_id']=='38')  
		       $silica_gel_series =$abcd;
		    else if($item['product_id']=='37')  
		       $oxygen_absorbers_series = $abcd;
		    else if($item['product_id']=='63')  
		       $val_series = $abcd;
		    else if($item['product_id']=='70')  
		       $straw_series = $abcd;
		...
		//sonu add 15-5-2017
		$f_p=$f_scoop=$f_roll=$f_mailer=$f_sealer=$f_storezo=$f_box=$f_con=$f_gls=$f_val=$f_chair=$f_oxygen_absorbers=$f_silica_gel=$f_straw=true;
		//end
		$first='false';$air_f = 0;
	...
	//	printr($air_f);
		...
		//	printr($net_pouches_p);
			...
			else if($details['product_id']=='70')
			{ 
				$tot_qty_straw=$this->getPlasticScoopQty($details['product_id'],$details['invoice_product_id'],$invoice_no);
				$tot_straw_qty = $tot_straw_qty + $tot_qty_straw['total']; 
				$tot_straw_rate = $tot_straw_rate + $tot_qty_straw['rate'];
				$total_amt_straw = $total_amt_straw + $tot_qty_straw['tot_amt'];
				$net_pouches_p = $this->getIngenBox($invoice_no,$details['product_id'],0);
				$total_net_w_straw = $net_pouches_p['n_wt'];
				//sonu add 15-5-2017
				
			//	print($details['product_id']);
				$total_gross_w_straw = $net_pouches_p['g_wt'];
				$straw_box_no = $net_pouches_p['total_box'];
				$straw_box_name = 'STRAW';
				//end
				$total_net_amt_straw = $net_pouches_p['total_amt'];
				$straw_no = '1';
				//$chair_series = 'C ';
				if($first=='false')
				{
			...
			else if($details['product_id']!='11' && $details['product_id']!='6' && $details['product_id']!='10' && $details['product_id']!='23' && $details['product_id']!='18' && $details['product_id']!='34' && $details['product_id']!='47' && $details['product_id']!='48' && $details['product_id']!='63' && $details['product_id']!='72' && $details['product_id']!='38' && $details['product_id']!='37' && $details['product_id']!='70')
			{
			   // printr($details['product_id']);
	  	...
	  	<tr><td colspan="'.$uk_colspan.'">';
        	 ...
        	 <tr>
      	     ...
      	     <tr>
                   ...
                   <span><img style="width:136px;height:60px;" class="barcode" alt="'.trim($invoice['invoice_no']).'" src="'.HTTP_SERVER.'admin/barcode/barcode.php?text='.trim($invoice['invoice_no']).'&codetype=Code128&orientation=horizontal&size=40&print=true"/></span>
        ...
        $html.=' </tr>';
		 ...
		 if($invoice['order_type']!='sample')
		 {
		 	...
		 	//printr($currency);
						...
						//Coffee Pouches
         		 			...
         		 			if($pouch_name=='POUCHES'){
    							...
    							if($invoice['country_destination']=='251'){
    						       ...
    						       if(str_replace(',', '', $cond_net_weight) >'200' && $invoice['invoice_date']>'2018-03-31' &&  $invoice['country_destination']!='169'  &&  $invoice['country_destination']!='77'){//printr($cond_net_weight);
						       ...
						       if(ucwords(decode($invoice['transportation']))=='Sea') 
								{  
							        /*
							        if($invoice['invoice_date']>'2019-12-11'){
							             $html.='<br><br><div><strong>This Shipment taken under the EPCG Licence <br> Licence No 3430003119 Dated 16.10.2017 </strong></div>';
							        }
							        else if($invoice['invoice_date']>'2018-10-21'){
							        ...
							        $html.='<br><div><strong>'.$invoice['license_text'].' </strong></div>';
								}
								     	...
								     	if(ucwords(decode($invoice['transportation']))=='Sea')
							            {
    								           $total_amt_val=$total_amt_val+$invoice['cylinder_charges'];
    								            if($invoice['invoice_date']<'2019-03-19'){	
                								    $amt = ($total_amt_val-($invoice['tran_charges']+$roll_price+$total_amt_paper+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_oxygen_absorbers+$total_amt_silica_gel+$total_amt_straw));
                							    }
                							    else
                							       	$amt = ($total_amt_val-($invoice['tran_charges']+$total_amt_roll+$total_amt_paper+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_silica_gel+$total_amt_oxygen_absorbers+$total_amt_straw));
                        								    ...
                        								    if($invoice['country_destination']=='172' || $invoice['country_destination']=='253' )//$invoice['country_destination']=='125' || 
                                								{
                                								    ...
                                								    $rate_per = number_format((($total_amt_val-($total_amt_paper+$roll_price+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_silica_gel+$total_amt_oxygen_absorbers+$total_amt_straw))/$total_qty_val),$rate_per_digit);
                                								    $amt = ($total_amt_val-($total_amt_paper+$roll_price+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_oxygen_absorbers+$total_amt_silica_gel+$total_amt_straw));
                                								}
                        							  }
                        						 }
                        					  $amt = ($total_amt_val-($invoice['tran_charges']+$total_amt_paper+$total_amt_roll+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_silica_gel+$total_amt_oxygen_absorbers+$total_amt_straw));
                                        }
                               ...
                               /* if($invoice['invoice_date']>'2020-02-09')
                                {
                                    $invoice['tran_charges']=$invoice['extra_tran_charges'];
                                }*/
                                
							
								if($scoop_no == '1')
								{	
								    $total_qty_val=$invoice_qty['total_qty']-($tot_scoop_qty+$tot_roll_qty+$tot_mailer_qty+$tot_sealer_qty+$tot_storezo_qty+$tot_paper_qty+$tot_con_qty+$tot_gls_qty+$tot_chair_qty+$tot_silica_gel_qty+$tot_oxygen_absorbers_qty+$tot_val_qty+$tot_straw_qty);
									$total_rate_val=$invoice_qty['total_rate']-($tot_scoop_rate +$tot_roll_rate +$tot_mailer_rate +$tot_sealer_rate +$tot_storezo_rate +$tot_paper_rate+$tot_con_rate +$tot_gls_rate+$tot_val_rate+$tot_chair_rate+$tot_silica_gel_rate+$tot_oxygen_absorbers_rate+$tot_straw_rate);
									$total_amt_val=$invoice_qty['tot'];
									
									if($air_f=='1'){
									       $total_amt_scoop = $total_amt_scoop - $invoice['tran_charges'];
									  if($invoice['show_air_f']==1)
									       $total_amt_scoop = $total_amt_scoop + $invoice['extra_tran_charges'];
								...
								{ 
									...
									$total_qty_val=$invoice_qty['total_qty']-($tot_scoop_qty+$tot_roll_qty+$tot_mailer_qty+$tot_sealer_qty+$tot_storezo_qty+$tot_paper_qty+$tot_con_qty+$tot_gls_qty+$tot_val_qty+$tot_chair_qty+$tot_silica_gel_qty+$tot_oxygen_absorbers_qty+$tot_straw_qty);
									$total_rate_val=$invoice_qty['total_rate']-($tot_scoop_rate +$tot_roll_rate +$tot_mailer_rate +$tot_sealer_rate +$tot_storezo_rate +$tot_paper_rate+$tot_con_rate +$tot_gls_rate+$tot_val_rate+$tot_chair_rate+$tot_silica_gel_rate+$tot_oxygen_absorbers_rate+$tot_straw_rate);
					                	if($pouch_name!='POUCHES'){  // change by 02-01-2020 for cylinder add in amount
								        	$total_amt_val=($invoice_qty['tot']-$roll_price)+$total_amt_roll+$invoice['cylinder_charges'];//printr($pouch_name); 
								        		if($air_f=='2'){
									                 $total_amt_roll = ($total_amt_roll + $invoice['cylinder_charges']) - $invoice['tran_charges'];
									                  if($invoice['show_air_f']==1)
									                   $total_amt_roll = $total_amt_roll + $invoice['extra_tran_charges'];
								        		}
								       } else{
								...
								if($mailer_no == '1')
								{
								  //  printr($invoice['tran_charges'].'&3');
									...
									if($air_f=='3'){
									     ...
									     if($invoice['show_air_f']==1)
									         $total_amt_mailer = $total_amt_mailer + $invoice['extra_tran_charges'];
									}
								}
								if($sealer_no == '1')
								{
								...
								//	printr($invoice['tran_charges'].'&4');
									...
									if($air_f=='4'){
								    	...
								    	if($invoice['show_air_f']==1)
									          $total_amt_sealer = $total_amt_sealer + $invoice['extra_tran_charges'];
								...
								//	printr($invoice['tran_charges'].'&5');
									    ...
									    if($air_f=='5'){
									        $total_amt_storezo = $total_amt_storezo - $invoice['tran_charges'];
									     	...
									     	if($invoice['show_air_f']==1)
									            $total_amt_storezo = $total_amt_storezo + $invoice['extra_tran_charges'];
									    }
								...
								//	printr($invoice['tran_charges'].'&6');
									...
									if($air_f=='6'){
									    ...
									    if($invoice['show_air_f']==1)
									            $total_amt_paper = $total_amt_paper + $invoice['extra_tran_charges'];
								...
								//	printr($invoice['tran_charges'].'&7');
									if($air_f=='7'){
									  ...
									  if($invoice['show_air_f']==1)
									            $total_amt_con = $total_amt_con + $invoice['extra_tran_charges'];
								...
								//	printr($invoice['tran_charges'].'&8');
									if($air_f=='8'){
									    ...
									    if($invoice['show_air_f']==1)
									            $total_amt_gls = $total_amt_gls + $invoice['extra_tran_charges'];
									}
									
								}
								if($chair_no == '1')
								{
									$total_qty_val=$invoice_qty['total_qty']-($tot_scoop_qty+$tot_roll_qty+$tot_mailer_qty+$tot_sealer_qty+$tot_storezo_qty+$tot_paper_qty+$tot_con_qty+$tot_silica_gel_qty+$tot_gls_qty+$tot_oxygen_absorbers_qty+$tot_val_qty+$tot_straw_qty);
									$total_rate_val=$invoice_qty['total_rate']-($tot_scoop_rate +$tot_roll_rate +$tot_mailer_rate +$tot_sealer_rate +$tot_storezo_rate +$tot_paper_rate +$tot_con_rate+$tot_gls_rate+$tot_val_rate+$tot_silica_gel_rate+$tot_oxygen_absorbers_rate+$tot_straw_rate);
									//$total_rate_val=$tot_paper_rate;
									$total_amt_val=$invoice_qty['tot'];
								//	printr($invoice['tran_charges'].'&8');
									if($air_f=='10'){
									    ...
									    if($invoice['show_air_f']==1)
									            $total_amt_chair = $total_amt_chair + $invoice['extra_tran_charges'];
									}
									
								}
								if($silica_gel_no == '1')
								{
									$total_qty_val=$invoice_qty['total_qty']-($tot_scoop_qty+$tot_roll_qty+$tot_mailer_qty+$tot_sealer_qty+$tot_storezo_qty+$tot_paper_qty+$tot_con_qty+$tot_chair_qty+$tot_gls_qty+$tot_oxygen_absorbers_qty+$tot_val_qty+$tot_straw_qty);
									$total_rate_val=$invoice_qty['total_rate']-($tot_scoop_rate +$tot_roll_rate +$tot_mailer_rate +$tot_sealer_rate +$tot_storezo_rate +$tot_paper_rate +$tot_con_rate+$tot_gls_rate+$tot_val_rate+$tot_oxygen_absorbers_rate+$tot_chair_rate+$tot_straw_rate);
									//$total_rate_val=$tot_paper_rate;
									$total_amt_val=$invoice_qty['tot'];
								//	printr($invoice['tran_charges'].'&8');
									if($air_f=='11'){
									    ...
									    if($invoice['show_air_f']==1)
									            $total_amt_silica_gel = $total_amt_silica_gel + $invoice['extra_tran_charges'];
									}
									
								}
								if($oxygen_absorbers_no == '1')
								{
									$total_qty_val=$invoice_qty['total_qty']-($tot_scoop_qty+$tot_roll_qty+$tot_mailer_qty+$tot_sealer_qty+$tot_storezo_qty+$tot_paper_qty+$tot_con_qty+$tot_chair_qty+$tot_gls_qty+$tot_silica_gel_qty+$tot_val_qty+$tot_straw_qty);
									$total_rate_val=$invoice_qty['total_rate']-($tot_scoop_rate +$tot_roll_rate +$tot_mailer_rate +$tot_sealer_rate +$tot_storezo_rate +$tot_paper_rate +$tot_con_rate+$tot_gls_rate+$tot_val_rate+$tot_silica_gel_rate+$tot_chair_rate+$tot_straw_rate);
									//$total_rate_val=$tot_paper_rate;
									$total_amt_val=$invoice_qty['tot'];
								//	printr($invoice['tran_charges'].'&8');
									if($air_f=='12'){
									        $total_amt_oxygen_absorbers = $total_amt_oxygen_absorbers - $invoice['tran_charges'];
									          if($invoice['show_air_f']==1)
									            $total_amt_oxygen_absorbers = $total_amt_oxygen_absorbers + $invoice['extra_tran_charges'];
									}
									
								}
								if($val_no == '1') 
								{
									$total_qty_val=$invoice_qty['total_qty']-($tot_scoop_qty+$tot_roll_qty+$tot_mailer_qty+$tot_sealer_qty+$tot_storezo_qty+$tot_paper_qty+$tot_con_qty+$tot_gls_qty+$tot_chair_qty+$tot_silica_gel_qty+$tot_straw_qty);
									$total_rate_val=$invoice_qty['total_rate']-($tot_scoop_rate +$tot_roll_rate +$tot_mailer_rate +$tot_sealer_rate +$tot_storezo_rate +$tot_paper_rate +$tot_con_rate+$tot_gls_rate+$tot_chair_rate+$tot_oxygen_absorbers_rate+$tot_silica_gel_rate+$tot_straw_rate);
									//$total_rate_val=$tot_paper_rate;
									$total_amt_val=$invoice_qty['tot'];
								//	printr($invoice['tran_charges'].'&9');
									if($air_f=='9'){
									        $total_amt_valve = $total_amt_valve - $invoice['tran_charges'];
									         if($invoice['show_air_f']==1)
									            $total_amt_valve = $total_amt_valve + $invoice['extra_tran_charges'];
									}
									
								}
								if($straw_no == '1') 
								{
									$total_qty_val=$invoice_qty['total_qty']-($tot_scoop_qty+$tot_roll_qty+$tot_mailer_qty+$tot_sealer_qty+$tot_storezo_qty+$tot_paper_qty+$tot_con_qty+$tot_gls_qty+$tot_val_qty+$tot_chair_qty+$tot_silica_gel_qty);
									...
									$total_amt_val=$invoice_qty['tot'];
								    if($air_f=='13'){
									        $total_amt_straw = $total_amt_straw - $invoice['tran_charges'];
									    if($invoice['show_air_f']==1)
									        $total_amt_straw = $total_amt_straw + $invoice['extra_tran_charges'];
								    }
                ...
                //printr($total_qty_val);
							//	printr($invoice_qty['total_rate']);
							if(ucwords(decode($invoice['transportation']))=='Sea')
							{
							   ...
							   $total_amt_val=$total_amt_val+$invoice['cylinder_charges']+$invoice['tool_cost']; // add tool cost told by pinank for invoice no EXP947 24-10-2019
								...
								$rate_per = number_format(( $amt/$total_net_w_pouch),$rate_per_digit); 
										//printr($rate_per);
										//printr($amt);
									//	printr($total_net_w_pouch);
									//	printr($rate_per_digit);
						    }
							else
							{
							    if($invoice['show_cylinder_air']=='1')
								{
                                    if($roll_no == '1' )
    						  	        $total_amt_roll=$total_amt_roll;
    						  	     else
    						  	        $total_amt_val=$total_amt_val+$invoice['cylinder_charges'];
								}
							     
							      if($invoice['show_air_f']==1)
									      $total_amt_val = $total_amt_val + $invoice['extra_tran_charges']+$invoice['tool_cost'] ;
                                
								$rate_per = number_format((($total_amt_val-($invoice['tran_charges']+$total_amt_roll+$total_amt_paper+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_oxygen_absorbers+$total_amt_silica_gel+$total_amt_straw))/$total_qty_val),$rate_per_digit);
						        $amt = ($total_amt_val-($invoice['tran_charges']+$total_amt_paper+$total_amt_roll+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_silica_gel+$total_amt_oxygen_absorbers+$total_amt_straw));
                               // printr($amt);
    							  ...
    							  if($invoice_no!='1899'){
    								    if($invoice['country_destination']=='172' || $invoice['country_destination']=='253' )//$invoice['country_destination']=='125' || 
            								{ 
            								    ...
            								    $rate_per = number_format((($total_amt_val-($total_amt_paper+$roll_price+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_silica_gel+$total_amt_oxygen_absorbers+$total_amt_straw))/$total_qty_val),$rate_per_digit);
            								    $amt = ($total_amt_val-($total_amt_paper+$roll_price+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls+$total_amt_valve+$total_amt_chair+$total_amt_silica_gel+$total_amt_oxygen_absorbers+$total_amt_straw));
            								}
    							  }
    							 }
							}	
						
							//	printr('('.$total_amt_val.'-('.$invoice['tran_charges'].'+'.$invoice['cylinder_charges'].'+'.$total_amt_paper.'+'.$total_amt_roll.'+'.$total_amt_scoop.'+'.$total_amt_mailer.'+'.$total_amt_sealer.'+'.$total_amt_storezo.'))/'.$total_qty_val.'),$rate_per_digit)');
    				 ...
    				 { 		 
    		          		...
    		          		if($invoice['rate_per_digit']==0)
    		          		{
              					if(ucwords(decode($invoice['transportation']))=='Sea')
              					{
              					   // printr($rate_per);
              					   // printr($total_net_w_pouch);
              					    $html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_pouch*$rate_per,2, '.', '').'</p>';
              					    $invoice_amount_total += number_format($total_net_w_pouch*$rate_per, 2, '.', '');
              					}
              					else
              					{
              					    $html.='<td class="no_border" valign="top"><p align="center">'.(number_format($total_qty_val*$rate_per,2, '.', '')).'</p>';
              					    $invoice_amount_total += (number_format($total_qty_val*$rate_per, 2, '.', ''));
              					}
              					
              					
              				}
              				else
              					$html.='<td class="no_border" valign="top"><p align="center">'.number_format($amt,2).'</p>';
    				 }
    				 else 
    				 {
    					 	 //[kinjal] cond added on 16-5-2017 
    				        $html.='<td colspan="2" class="no_border" valign="top" >';
    					 	if($scoop_no == '1' || $roll_no == '1' || $mailer_no == '1' || $sealer_no == '1' || $storezo_no == '1' || $paper_no == '1' || $con_no == '1' || $gls_no == '1' || $val_no =='1' || $chair_no =='1'|| $silica_gel_no =='1'|| $oxygen_absorbers_no =='1' || $straw_no =='1')
    					 	{
				...
				if($pouch_name=='POUCHES'){
					...
					//if($invoice['extra_tran_charges']!='0' && $invoice['tran_charges']='0.00')$html.='B) &nbsp;'. ucwords(decode($invoice['transportation'])).' Freight Charges';
							...
							$html.='<tr> 
										...
										if($status==1) 
										{
												if(($invoice['show_air_f']==0 ) && ($invoice['tran_charges']!='0' || $invoice['extra_tran_charges']!='0'))
													$html.=' &nbsp;'. ucwords(decode($invoice['transportation'])).' Freight Charges';
												
												if($invoice['cylinder_charges']!='0' && ucwords(decode($invoice['transportation']))!='Sea' && $invoice['show_cylinder_air']=='0')	
													$html.='<br>Cylinder Making Charges';
											    if($invoice['invoice_id']=='1809')
											        $html.='<br>Design Charges';
												if(($invoice['country_destination']=='252' ||  $invoice['order_user_id']==2 ) && $invoice['insurance_status']==0&& ucwords(decode($invoice['transportation']))=='Air')
												{
													$html.='<br>Insurance';
												}
											...
											$html.='<td class="no_border"></td>
													<td class="no_border"> 
														...
														if($invoice['invoice_id']=="2177"){
															...
															else if (($invoice['show_air_f']==0 ) && ($invoice['extra_tran_charges']!='0')) 	
															...
															$insurance='0';
															 ...
															 if(($invoice['order_user_id']==2 || $invoice['country_destination']=='252') && $invoice['insurance_status']==0 && ucwords(decode($invoice['transportation']))=='Air')
															  {
																$tran_charges_tot=round($invoice['tran_charges'],2)+$amt;
															//	printr($amt);
															//	printr($tran_charges_tot);
									...
									if($invoice['tool_cost'] !='0.00'  && ucwords(decode($invoice['transportation']))!='Sea' && $invoice['show_air_f']==0)
    								    { 
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br>HS CODE : 39201012<br>
									...
									if($status==1)
									{	
										    $html.='<td class="no_border" valign="top"><p align="center">'.number_format(($total_amt_roll)/$total_net_w_roll,$rate_per_digit).'</p></td>';
										    if($invoice['rate_per_digit']==0)
                    		          		{
                              					$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_roll*number_format(($total_amt_roll)/$total_net_w_roll,$rate_per_digit),2, '.', '').'</p>';
                              					$invoice_amount_total += number_format($total_net_w_roll*number_format(($total_amt_roll)/$total_net_w_roll,$rate_per_digit),2, '.', '');
                    		          		}
                              				else
								...
								if($mailer_no == '1')
								{   
								    if($invoice['order_user_id']==3 &&  $pouch_name!='POUCHES'){
									...
									$html.='<tr>
												<td colspan="2" class="no_border"></td>
												<td colspan="3" class="no_border"><div><strong>'.$mailer_series.') Mailer Bag</strong><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;HS CODE : '.$hs_code.'<br>
</div></td>';
												if(ucwords(decode($invoice['transportation']))=='Sea')
									...
									{	
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_mailer/$total_net_w_m,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_mailer/$tot_mailer_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_m*number_format($total_amt_mailer/$total_net_w_m,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_m*number_format($total_amt_mailer/$total_net_w_m,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_mailer_qty*number_format($total_amt_mailer/$tot_mailer_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_mailer_qty*number_format($total_amt_mailer/$tot_mailer_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
								...
								if($pouch_name!='POUCHES'){
									...
									$html.='<tr>
												<td colspan="2" class="no_border"></td>
												<td colspan="3" class="no_border"><strong>'.$scoop_series.') '.$text.' </strong><br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;HS CODE : 39235090
</td>';
												if(ucwords(decode($invoice['transportation']))=='Sea')
									...
									{	
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_scoop/$total_net_w_scoop,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_scoop/$tot_scoop_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_scoop*number_format($total_amt_scoop/$total_net_w_scoop,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_scoop*number_format($total_amt_scoop/$total_net_w_scoop,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_scoop_qty*number_format($total_amt_scoop/$tot_scoop_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_scoop_qty*number_format($total_amt_scoop/$tot_scoop_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else	
								...
								if($storezo_series=='A'){
									...
									{	
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_storezo/$total_net_w_str,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_storezo/$tot_storezo_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_str*number_format($total_amt_storezo/$total_net_w_str,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_str*number_format($total_amt_storezo/$total_net_w_str,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_storezo_qty*number_format($total_amt_storezo/$tot_storezo_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_storezo_qty*number_format($total_amt_storezo/$tot_storezo_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
								...
								if($sealer_no == '1')
								{
									...
									if($status==1)
									{
											...
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_sealer/$total_net_w_s,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_sealer/$tot_sealer_qty,$rate_per_digit).'</p></td>';
											}	
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_s*number_format($total_amt_sealer/$total_net_w_s,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_s*number_format($total_amt_sealer/$total_net_w_s,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_sealer_qty*number_format($total_amt_sealer/$tot_sealer_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_sealer_qty*number_format($total_amt_sealer/$tot_sealer_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;HS CODE : 48191010<br>
									...
									{	
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_paper/$total_net_w_p,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_paper/$tot_paper_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_p*number_format($total_amt_paper/$total_net_w_p,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_p*number_format($total_amt_paper/$total_net_w_p,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_paper_qty*number_format($total_amt_paper/$tot_paper_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_paper_qty*number_format($total_amt_paper/$tot_paper_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
								...
								//	  printr($total_amt_chair); 
									...
									{	   
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_oxygen_absorbers/$total_net_w_oxygen_absorbers,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_oxygen_absorbers/$tot_oxygen_absorbers_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_oxygen_absorbers*number_format($total_amt_oxygen_absorbers/$total_net_w_oxygen_absorbers,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_oxygen_absorbers*number_format($total_amt_oxygen_absorbers/$total_net_w_oxygen_absorbers,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_oxygen_absorbers_qty*number_format($total_amt_oxygen_absorbers/$tot_oxygen_absorbers_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_oxygen_absorbers_qty*number_format($total_amt_oxygen_absorbers/$tot_oxygen_absorbers_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
								...
								//	  printr($total_amt_chair); 
									...
									{	   
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_silica_gel/$total_net_w_silica_gel,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_silica_gel/$tot_silica_gel_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_silica_gel*number_format($total_amt_silica_gel/$total_net_w_silica_gel,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_silica_gel*number_format($total_amt_silica_gel/$total_net_w_silica_gel,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_silica_gel_qty*number_format($total_amt_silica_gel/$tot_silica_gel_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_silica_gel_qty*number_format($total_amt_silica_gel/$tot_silica_gel_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;HS CODE : 39241090<br>
									...
									{	
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_con/$total_net_w_con,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_con/$tot_con_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_con*number_format($total_amt_con/$total_net_w_con,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_con*number_format($total_amt_con/$total_net_w_con,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_con_qty*number_format($total_amt_con/$tot_con_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_con_qty*number_format($total_amt_con/$tot_con_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;HS CODE : 39241090<br>
									...
									{	
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_gls/$total_net_w_gls,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_gls/$tot_gls_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_gls*number_format($total_amt_gls/$total_net_w_gls,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_gls*number_format($total_amt_gls/$total_net_w_gls,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_gls_qty*number_format($total_amt_gls/$tot_gls_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_gls_qty*number_format($total_amt_gls/$tot_gls_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
								...
								if($val_no == '1')
								{
									...
									{	
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_valve/$total_net_w_val,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_valve/$tot_val_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_val*number_format($total_amt_valve/$total_net_w_val,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_val*number_format($total_amt_valve/$total_net_w_val,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_val_qty*number_format($total_amt_valve/$tot_val_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_val_qty*number_format($total_amt_valve/$tot_val_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
								...
								//	  printr($total_amt_chair); 
									...
									{	   
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_chair/$total_net_w_chair,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_chair/$tot_chair_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_chair*number_format($total_amt_chair/$total_net_w_chair,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_chair*number_format($total_amt_chair/$total_net_w_chair,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_chair_qty*number_format($total_amt_chair/$tot_chair_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_chair_qty*number_format($total_amt_chair/$tot_chair_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
								...
								if($straw_no == '1')
								{
									$html.='<tr>
												<td colspan="2" class="no_border"></td>
												<td colspan="3" class="no_border"><div><strong>'.$straw_series.') STRAW</strong><br>
												&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;HS CODE : 48239090<br></div></td>';
												if(ucwords(decode($invoice['transportation']))=='Sea')
													$html.='<td class="no_border" valign="top"><p align="center">NET. WT.<br>'.$total_net_w_straw .'<br>'.$tot_straw_qty.' Nos </p></td>';
												else
												    $html.='<td class="no_border" valign="top"><p align="center">'.$tot_straw_qty.'</p></td>';
								...
								//	  printr($total_amt_chair); 
									...
									{	   
											if(ucwords(decode($invoice['transportation']))=='Sea')
											{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_straw/$total_net_w_straw,$rate_per_digit).'</p></td>';
											}else{
												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_straw/$tot_straw_qty,$rate_per_digit).'</p></td>';
											}
											if($invoice['rate_per_digit']==0)
                    		          		{
                              					if(ucwords(decode($invoice['transportation']))=='Sea')
    											{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_net_w_straw*number_format($total_amt_straw/$total_net_w_straw,$rate_per_digit),2, '.', '').'</p></td>';
    												$invoice_amount_total += number_format($total_net_w_straw*number_format($total_amt_straw/$total_net_w_straw,$rate_per_digit),2, '.', '');
    											}else{
    												$html.='<td class="no_border" valign="top"><p align="center">'.number_format($tot_straw_qty*number_format($total_amt_straw/$tot_straw_qty,$rate_per_digit),2, '.', '').'</p></td>';
    											    $invoice_amount_total += number_format($tot_straw_qty*number_format($total_amt_straw/$tot_straw_qty,$rate_per_digit),2, '.', '');	
    											}
    										}
                              				else
											    $html.='<td class="no_border" valign="top"><p align="center">'.number_format($total_amt_straw,2).'</p></td>';
									}
									else
										$html.='<td class="no_border" colspan="2"><br>'.$straw_box_no.' BOXES OF '.$straw_box_name.'<br> Total GWT : '.$total_gross_w_straw.'KGS 
												<br> Total NWT :'.$total_net_w_straw.'KGS</td>';
								...
								if($invoice['rate_per_digit']==0)
								{
								    $total_amt_val=$invoice_amount_total+$invoice['tran_charges'];
								}
...
The exporter <strong>"INREX3403000290EC005"</strong> of the products covered by this document declares that, except where otherwise clearly indicated, these <br/>products are of Indian preferential origin according to rules of origin of the Generalized System of Preferences of the European Union and that the origin criterion met is P.</div>';
										 ...
										 $html.='<div><strong>We here by declare that, we shall claim as admissible <br></strong> under the chapter 3 FTP</div>';
       ...
       //printr($total_amt_val);
		$final_amount=$total_amt_val+$insurance+$sri_charge;//printr($final_amount);
		$excies = '';
		$tax ='';
	...
	else
	{
		...
		$firsttime = true;
			...
			$f_p=$f_scoop=$f_roll=$f_mailer=$f_sealer=$f_storezo=$f_box=$f_straw=$f_con=$f_gls=$f_chair=$f_silica_gel=$f_oxygen_absorbers=$f_val=true;
			$alldetails=$this->getProductdeatils($invoice_no);
			//printr($alldetails);
			foreach($alldetails as $detail)
											{
												if($detail['product_id']!='11' && $detail['product_id']!='6' && $detail['product_id']!='10' && $detail['product_id']!='23' && $detail['product_id']!='18' && $detail['product_id']!='34' && $detail['product_id']!='47' && $detail['product_id']!='48'  && $detail['product_id']!='63' && $detail['product_id']!='72' && $detail['product_id']!='70')
												{
												...
												else if($detail['product_id']=='70')
												{
													$boxDetail = $this->getIngenBox($invoice_no,$detail['product_id']);
													$boxDetail['product_id']=$detail['product_id'];
													if($f_straw==true)
													{
														$pouch_detail['Straw']=$boxDetail;
														$f_straw=false;
													}
												}
											}
...
//	  printr($Total_price); 
	if((ucwords(decode($invoice['transportation']))=='Air' || ucwords(decode($invoice['transportation']))=='Road' ) && $invoice['show_cylinder_air']=='0')
		$Total_price = $final_amount = $final_amount + $invoice['cylinder_charges'];

	//	  printr($invoice['show_cylinder_air']);
	...
	if(($invoice['country_destination']=='172' || $invoice['country_destination']=='253') && ucwords(decode($invoice['transportation']))=='Air')//$invoice['country_destination']=='125' || 
         ...
         $html .='<tr>
                    ...
                    if($status==1){
                    					...
                    					if($invoice['show_air_f']==0 ){
                    				    $html.='<td><p align="center">'; 
                    				    	$html.=$this->numberFormate(($final_amount+$invoice['extra_tran_charges']+$invoice['tool_cost']),"2").'</p></td>';
                    					}else{
                    					     $html.='<td><p align="center">'; 
                    				    	$html.=$this->numberFormate(($final_amount),"2").'</p></td>';
                    					}
                    				    $html.='</tr>';
                    				    if($invoice['show_air_f']==0 )
                    		...
                    		if($invoice['country_destination'] ==111 ) {
                             ...
                             $html .='<tr>';
                    						...
                    						if($invoice['country_destination']=='238')
                    					    	$html.='<b>FOB VALUE = '.number_format((($final_amount)-$invoice['tran_charges']-$invoice['FOB_value']),2).' USD</b>';
                    						$html.='</td>
                             	</tr>';
                }else {
																...
																<table  style="" id="innerbox" class="innerbox" style="max-width:550px; height:300px;display: inline-block;" >
																	...
																	<tr>
																		...
																		if( $invoice['country_destination'] == '253')
            																{  if($val['client_no']!=''){
                                                                               	$setHtml .='<tr>
                																		<td  style="padding:0px;border:0px">CLIENT NO  &nbsp;:&nbsp;</td>
                																		<td  style="padding:0px;border:0px">'.$val['client_no'].'</td>
                																	</tr>';
                                                                             }
                                                                                	$setHtml .='<tr>
            																		<td  style="padding:0px;border:0px">ORDER NO &nbsp;:&nbsp;</td>
            																		<td  style="padding:0px;border:0px">'.$val['ref_no'].'</td>
            																	</tr>';
        											...
        											$setHtml .='<td style="width:33%;border:none; border-top:none;">
													    	...
													    	if( $invoice['country_destination'] == '253')
    																      ...
    																      if($ch['client_no']!=''){
                                                                               	$setHtml .='<tr>
                																		<td  style="padding:0px;border:0px">CLIENT NO  &nbsp;:&nbsp;</td>
                																		<td  style="padding:0px;border:0px">'.$ch['client_no'].'</td>
                																	</tr>';
                                                                             }
                                                                                	$setHtml .='<tr>
            																		<td  style="padding:0px;border:0px">ORDER NO &nbsp;:&nbsp;</td>
            																		<td  style="padding:0px;border:0px">'.$ch['ref_no'].'</td>
            																	</tr>';
							...
							<div class="table-responsive"><br>
									<div style="width:730px;">
										<table class="table b-t table-striped text-small table-hover detail_table"> 
											...
											$f_p=$f_scoop=$f_roll=$f_mailer=$f_sealer=$f_storezo=$f_box=$f_con=$f_oxygen_absorbers=$f_silica_gel=$f_chair=$f_val=$f_straw=true;
											foreach($alldetails as $detail)
											{
												if($detail['product_id']!='11' && $detail['product_id']!='6' && $detail['product_id']!='10' && $detail['product_id']!='23' && $detail['product_id']!='18' && $detail['product_id']!='34' && $detail['product_id']!='47' && $detail['product_id']!='48' && $detail['product_id']!='63' && $detail['product_id']!='72' && $detail['product_id']!='37' && $detail['product_id']!='38' && $detail['product_id']!='70')
												{
							...
							<div class="table-responsive"><br>
									<div style="width:730px;">
										<table class="table b-t table-striped text-small table-hover detail_table"> 
											...
											$f_p=$f_scoop=$f_roll=$f_mailer=$f_sealer=$f_storezo=$f_box=$f_con=$f_val=$f_chair=$f_straw=true;
											foreach($alldetails as $detail)
											{
...
function viewConsolidatedSheet($invoice_no)
	...
	//	printr($consol_data);die;
		...
		{	
		    //$childBox=$this->query("SELECT * FROM in_gen_invoice_test WHERE parent_id IN (".$dat['grouped_in_gen_invoice_id'].") AND is_delete=0 AND rack_done_status=0");
			...
			$total_box_arr1=explode(',',$dat['grouped_box_no']);
			
		    $total_box_arr=array_filter($total_box_arr1);
          ...
          $arr = $remain= array();
            ...
            {   //array_search($b, array_column($userdb, 'uid'));
                if($k==$b)
                {
                    $arr[]=$k;
                    $k=$k+1;
                ...
                {
                    $remain[]=$b;
                    $sql ="SELECT ip.invoice_product_id,ig.in_gen_invoice_id, COUNT(DISTINCT ig.in_gen_invoice_id) AS total_boxes, GROUP_CONCAT(ig.box_no) AS grouped_box_no, SUM(ig.qty) AS qty,SUM(ig.box_weight+ig.net_weight) AS gross_weight,SUM(ig.net_weight) AS net_weight  ,SUM(ic.rate) AS rate,ip.item_no,ip.zipper,ip.valve,p.product_name,p.product_id,ic.size,tm.measurement,pc.color,ic.color_text,ic.dimension,ip.ref_no,ip.buyers_o_no FROM in_gen_invoice_test AS ig , invoice_product_test AS ip,product AS p,invoice_color_test AS ic,template_measurement AS tm,pouch_color AS pc WHERE ig.invoice_id='".$invoice_no."' AND ig.is_delete=0 AND ig.invoice_product_id=ip.invoice_product_id AND ip.product_id=p.product_id AND ig.invoice_color_id=ic.invoice_color_id AND ic.measurement=tm.product_id AND ic.color=pc.pouch_color_id AND ig.box_no IN (".$b.")";
        		    $data = $this->query($sql);
			...
			if($dat['size']=='' || $dat['size']=='0'  || $dat['size']=='0 kg' || $dat['size']==' kg'  || $dat['size']==' gm'  )
    			...
    			foreach($remaining as $rem)
    		    {
                    ...
                    {        //  printr($find_in_arr.'find_in_arr');printr($other_box.'other_box');   
            		  ...
            		  // printr($data->row);
            		    ...
            		    if($data->row['size']=='' || $data->row['size']=='0' || $data->row['size']=='0 kg'  || $data->row['size']==' kg'  || $data->row['size']==' gm' )
			...
			$tot_box = $tot_box +count($total_box_arr);
			$tot_cost = $tot_cost + $cost;
			//printr($dat['total_boxes']);
		}
        //sort($remaining);
		...
		if($invoice['extra_tran_charges']!=0.00)
		{
		    $setHtml.='<tr>
    						...
    						<td style="text-align:center" colspan="3">Extra Air Fright Charges	</td>
    						<td style="text-align:center" colspan="3"></td>
    						<td style="text-align:center" colspan="5">'.number_format($invoice['extra_tran_charges'],2).'</td>
    					</tr>';
    		$total_cost = $total_cost + $invoice['extra_tran_charges'];
		}
		$setHtml.='<tr>
						<td></td>
						<td></td>
...
function viewAddress($invoice,$status='')
	...
	//	printr($filter_data);die;
		...
		if(!empty($filter_data)){
			...
			if($filter_data['transport'] != ''){
				$sql .= " AND i.transportation LIKE '%".$filter_data['transport']."%' "; 	
			}
...
//	printr($data['con_invoice_id']);
		...
		$currency=$this->getCurrencyName($invoice['curr_id']);
		 ...
		 $measurement=$this->getMeasurementName($invoice['measurement']);
				if($scoop_no == '1')
								...
								$rate_per = number_format((($total_amt_val-($invoice['tran_charges']+$invoice['cylinder_charges']+$total_amt_paper+$total_amt_roll+$total_amt_scoop+$total_amt_mailer+$total_amt_sealer+$total_amt_storezo+$total_amt_con+$total_amt_gls))/$total_qty_val),$rate_per_digit);
	...
	public function getIngenBox($invoice_id,$product_id=0,$charge=0)
	{
		...
		if($product_id!='2')
			...
			$pro_id= " AND ip.product_id NOT IN (10,23,11,18,6,34,47,48,72,63,37,38,70)";
...
//	printr($data_pro);
	...
	//	printr($sql1);
		...
		//	printr($data_pro->row['total_rate'].'*'.$data->row['net']);
			//return $data->row;
			$d = array( 'total_box'=>$data->row['total_box'],
						 'qty' => $data_pro->row['total_qty'],
						 'g_wt' => $data1->row['gross'],
						 'n_wt' => $data1->row['net'],
						 'total_amt' =>$data_pro->row['total_rate']*$data1->row['net'],
						 'total_rate' =>$data_pro->row['total_rate'],
						 'roll_net' =>$data->row['net'],
...
//		echo $sql;die;
	...
	public function viewFullSalePurReport($post,$n=0)
	{
	    $product_code='';
	    ...
	    if($_SESSION['LOGIN_USER_TYPE'] == 1 && $_SESSION['ADMIN_LOGIN_SWISS'] == 1){
    		$sql_pur = "SELECT pc.product_code_id,pc.product_code,ic.qty,ic.rate,i.customer_dispatch,i.customer_name,i.invoice_no,pc.description,i.invoice_date,ic.color_text,ip.digital_print_color FROM  " . DB_PREFIX . "invoice_test as i,invoice_product_test as ip,product_code as pc,invoice_color_test as ic WHERE i.invoice_id =ip.invoice_id ".$product_code." AND pc.product_code_id=ip.product_code_id AND i.is_delete=0 AND ip.invoice_product_id = ic.invoice_product_id AND i.invoice_id=ic.invoice_id  AND i.invoice_date >= '".$post['f_date']."' AND i.invoice_date <='". $post['t_date'] ."' AND i.import_status=2";
    	    $sql_sale = "SELECT pc.product_code_id,pc.product_code,ip.qty,ip.rate,i.customer_dispatch,i.customer_name,i.invoice_no,pc.description,i.invoice_date,ip.stock_print as digital_print_color FROM  " . DB_PREFIX . "sales_invoice as i,sales_invoice_product as ip,product_code as pc WHERE i.invoice_id =ip.invoice_id  ".$product_code." AND pc.product_code_id=ip.product_code_id AND i.is_delete=0 AND i.invoice_date >= '".$post['f_date']."' AND i.invoice_date <='". $post['t_date'] ."'";
		} else {
		    ...
		    $html .= '<div class="table-responsive">';
    			 ...
    			 $html .= ' <thead>';
    	                ...
    	                $html.='<tr>
    	                    ...
    	                    <tbody>';
    	                    $amount = $qty=$i=0;$j=1;$total_outward=$total_inward=0;
                		    ...
                		    { //printr($array);
                		        ...
                		        $html .='<tr '.$style.'>
                    		                    ...
                    		                    if( strpos( $array['invoice_no'], 'EXP' ) !== false){
                    		                        ...
                    		                        $total_inward=$total_inward+$array['qty'];
                    		                    }else{
                    		                        $status='Receivable Invoice'; 
                    		                         $total_outward=$total_outward+$array['qty'];
                    		                    }
                    		                    $customer_dis='';    
                    		                  if($array['customer_dispatch']==1)
                    		                       $customer_dis='<b><br><small style="color:red;" >(Direct dispatch to customer.)</small></b>';
                    		                  
                    		              $print='';
                    		              if($array['digital_print_color']!='' && $array['digital_print_color']=='Digital Print')
                    		                $print='<span style="color:red;">( Digital Print Order )</span>';
                    		           $html .='<td>'.$status.'</td>
                    		                    <td>'.$array['invoice_no'].' '.$customer_dis.'</td>
                		    ...
                		    $html.='<tr>
                		                ...
                		                <td></td>
                		                <td>'.$amount.'</td>
                		                <td>'.$qty.'(<b>[Inward]=>'.$total_inward.'[Outward]=>'.$total_outward.'</b>)</td>
                		            </tr>';
                    $html .= '</tbody>
                          </table>';
            $html .= ' </div>';
  ...
  function viewDetailsDatatable($invoice_no,$status=0,$price=0,$p_break=0,$option=array(),$show_status='0')
	...
	$invoice_qty=$this->getInvoiceTotalData($invoice_no);
		...
		$cylinder_array=array();
		 $cylinder_details = "SELECT ic.*,ip.*  FROM  " . DB_PREFIX . "invoice_test as i,invoice_color_test as ic,invoice_product_test as ip WHERE  i.invoice_id =ic.invoice_id AND i.invoice_id =ip.invoice_id AND ip.invoice_product_id=ic.invoice_product_id AND i.invoice_id = '" .(int)$invoice_no. "' AND i.is_delete=0 AND cylinder_rate!=0.00 AND no_of_cylinder!=0 AND ic.color='-1'";
		$cylinder_data = $this->query($cylinder_details);
	...
	public function Product_detailTest($invoice_id,$invoice_product_id,$invoice_color_id)
	{
		$b_no=""; 
		  ...
		  $sql="select ii.qty,ii.size,ii.rate,tm.measurement,ii.invoice_product_id,p.product_name,p.product_id,ii.invoice_color_id,ip.valve,ip.zipper,ip.buyers_o_no,ip.client_no,ip.filling_details,ip.ref_no,ip.item_no,ii.net_weight,ii.invoice_id,ii.dis_rate,ii.no_of_cylinder,ii.cylinder_rate,ii.dimension,c.color,ii.color_text,c.pouch_color_id from invoice_color_test as ii,pouch_color as c,template_measurement as tm,invoice_product_test as ip,product as p where c.pouch_color_id=ii.color AND tm.product_id=ii.measurement AND ii.invoice_product_id=ip.invoice_product_id AND ip.product_id=p.product_id  AND ip.invoice_product_id='".$invoice_product_id."' AND ii.invoice_color_id='".$invoice_color_id."'"; 
   ...
   function send_mail_customer($data,$file){
    ...
    public function getorder_qty_vs_dispatch_qty($invoice_id){
     //   $sql="SELECT ip.* FROM  " . DB_PREFIX . "invoice_test as i,invoice_product_test as ip,template_order_test as t,stock_order_dispatch_history_test as s,product_code as p WHERE  ip.product_code_id=p.product_code_id  AND t.product_code_id=ip.product_code_id AND s.template_order_id=t.template_order_id AND t.template_order_id=ip.order_id AND t.is_delete=0 AND ip.is_delete=0 AND  i.invoice_id =ip.invoice_id  AND i.invoice_id = '" .(int)$invoice_id. "' AND i.is_delete=0";
       // $sql="SELECT ip.* FROM  " . DB_PREFIX . "invoice_test as i,invoice_product_test as ip,template_order_test as t,stock_order_dispatch_history_test as s,product_code as p WHERE  ip.product_code_id=p.product_code_id  AND t.product_code_id=ip.product_code_id AND s.template_order_id=t.template_order_id AND t.template_order_id=ip.order_id AND t.is_delete=0 AND ip.is_delete=0 AND  i.invoice_id =ip.invoice_id  AND i.invoice_id = '" .(int)$invoice_id. "' AND i.is_delete=0";
   
        $sql1="SELECT  ip.*, t.template_order_id, t.quantity as order_qty, d.dis_qty,ic.qty,p.product_code FROM invoice_product_test as ip LEFT JOIN template_order_test as t ON t.template_order_id = ip.order_id LEFT JOIN  stock_order_dispatch_history_test as d  ON d.template_order_id= ip.order_id LEFT JOIN product_code as p ON p.product_code_id =ip.product_code_id LEFT Join invoice_color_test as ic ON ic.invoice_product_id=ip.invoice_product_id  WHERE ip.invoice_id='".$invoice_id."' AND ip.is_delete=0 AND ip.buyers_o_no NOT LIKE'%CUST%'";
    //printr($sql1);//die;
